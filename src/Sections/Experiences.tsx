  ...
  const content = (
    <motion.ul className="flex flex-col gap-8" variants={itemsVariants}>
      {experiencesData.map((experience) => (
        <motion.li
          key={experience.id}
          className="flex flex-col gap-4 text-ms"
          variants={itemsVariants}>
          <h3>
            <a className="underline font-semibold" href={experience.siteUrl} target="_blank">
              {experience.name} - {experience.company}
            </a>
          </h3>
          <span className="text-secondary-foreground text-xs font-normal">{experience.date}</span>
          <p className="text-muted-foreground font-normal">{experience.description}</p>
